...
final class DailyNotificationContentService {
    ...
    private func generateAIContent(for date: Date) async -> (title: String, body: String)? {
        ...
        【当日能量背景】（仅供你感知这一天的气场，不要在文案中直接说出来）
        - 节气：\(jieQiStr)（对应的自然意象与能量特质）
        - 星期：\(weekday)（周期节律，影响人的行动/休整状态）
        - 农历：\(lunarDateStr)（月相与干支能量）
        - 用户命局：\(elementStr)
        - 近期关注：\(recentTopic)

        【文案写作要求】
        1. 标题：≤12个字，含一个emoji，像老友一句话，有温度有力量
        2. 正文：≤45个字，写一个"此刻的感受、提醒或鼓励"，让用户感觉"恰好说中了"
        3. 用能量意象和具体行动说话，禁止出现：周几、今日、明日、昨日、具体节气名称、农历日期、"你的命局"等字样
        4. 读起来像朋友发来的一句话，不像日历提醒，不像算命先生
        5. 输出格式（只输出JSON，不要其他内容）：
           {"title":"...","body":"..."}
        """
